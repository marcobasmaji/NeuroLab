...
TEST(OpenVino, CombinationOfHardware)
{
    ...
    EXPECT_EQ("MULTI:CPU,MYRIAD",distribtuion.back().first);
    EXPECT_EQ(13,distribtuion.back().second);
    EXPECT_EQ(3,distribtuion.front().second);
    EXPECT_EQ("CPU",distribtuion.front().first);
